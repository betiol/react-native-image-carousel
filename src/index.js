...
const screenHeight = screenSize.height - Platform.select({ ios: 0, android: StatusBar.currentHeight });
...
class ImageCarousel extends React.Component<PropsType, StateType> {
	...
	carouselItems: $ReadOnlyArray<?React.Element<*>> = Array.isArray(this.props.children)
	...
	renderFullscreen = () => {
		...
		return (
			<Modal transparent visible={fullscreen} onShow={this.handleModalShow} onRequestClose={this.close}>
				...
				{footer && <Animated.View style={[ opacity, styles.footerContainer ]}>{footer}</Animated.View>}
			</Modal>
		);
	};
